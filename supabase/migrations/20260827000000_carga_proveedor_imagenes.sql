-- ============================================================================
-- CARGA: imagenes de proveedores (documento "Hoteles Foto" / bucket Storage
-- "Fotos Hoteles")
--
-- El usuario subio 149 fotos al bucket "Fotos Hoteles" con el mismo nombre
-- de hotel que aparece en el documento "Hoteles Foto" (nombre_md), la misma
-- fuente que ya se cargo en staging_proveedor_descripciones. Se resuelve el
-- proveedor_id en dos pasos: (1) por el nombre_md exacto contra
-- staging_proveedor_descripciones (ya trae proveedor_id/proveedor_id_sugerido
-- resuelto), (2) por nombre exacto (case-insensitive) contra proveedores para
-- los casos que no aparezcan en el staging de descripciones.
-- ============================================================================
  ...
  select
    ...
    coalesce(
      ...
      (
        select p.id
        from proveedores p
        where lower(p.nombre) = lower(f.nombre_md)
        limit 1
      )
    ) as proveedor_id
  from stg_fotos f
...
where r.proveedor_id is not null
  and not exists (
    ...
    where pi.proveedor_id = r.proveedor_id
      and pi.storage_path = r.storage_path
  );

-- Reporte de fotos que no encontraron proveedor (revisar manualmente:
-- probablemente el nombre_md no coincide exactamente con proveedores.nombre
-- ni con staging_proveedor_descripciones.nombre_md).
do $$
declare
  sin_match text;
begin
  ...
  where not exists (
    ...
    from staging_proveedor_descripciones d
    where d.nombre_md = f.nombre_md
      and coalesce(d.proveedor_id, d.proveedor_id_sugerido) is not null
  )
  and not exists (
    select 1 from proveedores p where lower(p.nombre) = lower(f.nombre_md)
  );
